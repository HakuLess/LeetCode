...
class Solution5753 {
    fun largestPathValue(colors: String, edges: Array<IntArray>): Int {
        ...
        if (g.topologicalSort().isEmpty()) return -1

        val seen = HashSet<Int>()
        var ans = 0
        fun dfs(cur: Int, map: HashMap<Char, Int>) {
            if (cur in seen) return
//            seen.add(cur)
            map[colors[cur]] = map.getOrDefault(colors[cur], 0) + 1
            if (g.adj[cur].isEmpty()) {
                ans = maxOf(ans, map.values.max()!!)
            }
            g.adj[cur].forEach {
                dfs(it, map.clone() as HashMap<Char, Int>)
            }
        }

        val inMap = HashMap<Int, Int>()
        edges.forEach {
            inMap[it[1]] = inMap.getOrDefault(it[1], 0) + 1
        }
        val st = ArrayList<Triple<Int, LinkedList<Int>, Int>>()
        g.adj.mapIndexed { index, linkedList ->
            st.add(Triple(index, linkedList, inMap.getOrDefault(index, 0)))
        }
        st.filter { it.third == 0 }.forEach {
            dfs(it.first, hashMapOf())
        }
        return ans
    }
}