...
fun main(args: Array<String>) {
    val s = Solution4()
    s.findMedianSortedArrays(intArrayOf(1, 3), intArrayOf(2)).print()
}
...
class Solution4 {
    ...
    private fun findKSmall(nums1: IntArray, nums2: IntArray, start1: Int, start2: Int, k: Int): Double {
        val m = nums1.size
        val n = nums2.size

        if (n - start2 < m - start1) return findKSmall(nums2, nums1, start2, start1, k)
        if (m - start1 == 0) return nums2[start2 + k - 1].toDouble()
        if (k == 1) return minOf(nums1[start1], nums2[start2]).toDouble()

        val pos1 = minOf(k / 2, m)
        val pos2 = k - pos1
        return when {
